...
class RecipeRecommendation:
    ...
    def __init__(self):
        self.df = pd.read_csv('mini_recipes.csv')

        self.required_ingredients = {}

        for i, e in enumerate(self.df['title']):
            key = e
            ingredients_for_receipe = set(json.loads(self.df['NER'][i]))
            self.required_ingredients[key] = ingredients_for_receipe
    
    def match_ingredients(self, list_of_required_ingredients):
        #assumed that formatting is done correctly, comes in a list of strings that contatins all ingredients
        score = []
        for k, v in self.required_ingredients.items():
            counter = 0
            for e in list_of_required_ingredients:
                if e in v:
                    counter += 1
            heapq.heappush(score, (-1 * counter, k))
        
        return heapq.heappop(score)


if __name__ == "__main__":
    rr = RecipeRecommendation()
    print(rr.match_ingredients(json.loads("[\"bite size shredded rice biscuits\", \"vanilla\", \"brown sugar\", \"nuts\", \"milk\", \"butter\"]")))
...
# def match_ingredients(user_ingredients):
    
#     matched_ingredients = [] 

#     for ingredient in user_ingredients:
#         match, score = process.extractOne(ingredient, processed_ingr)
...
# input = match_ingredients(user_ingredients)
